...
def main():
    ...
    linewidth = 10
    TE = 30
    base_path = "/home/stud/casperc/bhome/Project1_DeepFit"
    raw_files_path = f"DeepFit-data-generation/DeepFitData_TE_{TE}_lw{linewidth}_2048p_jmrui"
    now = time.strftime("%Y_%m_%d-%H_%M_%S")

    # Define the format to save the data in, and the folder name to save it in
    save_format = "npz" # "mat" or "npz"

    save_folder = f"generated_data/{now}-standard_amplitude_{save_format}_TE{TE}_lw{linewidth}"
    ...
    for i in tqdm(range(start_val+1,start_val+num_spectra+1)):
        ...
        if BPF_filtering:
            ...
            ftype = "butter"
            pass_band_gain = 2
            filtered_signal, fft_filtered_signal, impulse_response = BPF_ppm(augmented, ppm_range=ppm_range, ppm_pass=passband, ppm_stop=stopband, gain_passband=pass_band_gain, ftype=ftype)
        else:
            filtered_signal = augmented
            fft_filtered_signal = fftshift(fft(filtered_signal))
            impulse_response = np.zeros(2048)
            passband = np.zeros(2)
            stopband = np.zeros(2)
            ftype = None
        metadata_list = [SNR, n_gaussians, STD_RANGE, AMPLITUDE_RANGE, max_val, min_val, optimizer_options, passband, stopband,ftype, ppm_range, linewidth]
        metadata_keys = ["SNR", "n_gaussians", "STD_RANGE", "AMPLITUDE_RANGE", "max_val", "min_val", "optimizer_options", "passband", "stopband", "ftype", "ppm_range", "linewidth"]   
        ...
        if save_format == "mat":
            ...
            savedict = {
                ...
                "original_ifft": original_ifft,
                "iir_order": iir_order,
                "filter_impulse_response": impulse_response,
            ...
            for key in metabolite_FID.keys(): 
                savedict[key] = metabolite_FID[key]
            np.savez(os.path.join(base_path, save_folder, f"{i_str}_data.{save_format}"), **savedict)
