...
import re
from myfunctions_tf import BPF_ppm, load_from_directory, multi_channel_cnn, ComplexFilterOptimizer, input_ouput_key_sorting, model_input_output_prep, load_mrui_txt_data, postprocess_predicted_data, read_LCModel_table, build_mrs_unet_1d
import pandas as pd
import pickle
...
def process_and_save(
    ...
    model = densenet(input_shape=(2048, 2*len(input_keys)), output_shape=(2048, 2*len(output_keys)), f=16)
...
# File paths
raw_data_base = "/home/stud/casperc/bhome/Project3_DL_sigpros/in_vivo_data_2/correlation_analysis"
        ...
        for folder, data in data_dict_HLSVD.items():
            for subject, lcm_data in data["lcm"].items():
                ...
                for met, values in lcm_data.items():
                    global_max_concentration_lcm = max(global_max_concentration_lcm, values["conc"])
...
for config, data_dict_HLSVD in all_configs_data.items():
    ...
    results_df.to_csv(f"correlations_2/{config}_HLSVD_correlation_results_raw.csv", index=False)
    
    # Save the formatted results for display
    formatted_df.to_csv(f"correlations_2/{config}_HLSVD_correlation_results.csv", index=False)